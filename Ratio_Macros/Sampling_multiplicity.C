// Example displaying two histograms and their ratio of Multiplicity / sample[i]
// Author: Helene Ausar
void Sampling_multiplicity() {
   ...
   // Define the Canvas
    ...
    //==========================================================================
//******************************************************************************
//..Multiplicity and 10 random samples:
//******************************************************************************
//Open root.file:
    TFile *file = TFile::Open("/Users/Helena/Dropbox/0-Speciale/code/panda/Thesis_Macros/bar_GF/Helene_WNUA/merging/Test_sampling_Multiplicity.root", "READ");
    TDirectory *dir = (TDirectoryFile*)file->Get("default");
    TList *list = (TList*)dir->Get("Flow_Refs_default");

    TH1F *h1 = (TH1F*)list->FindObject("fMultfBin_number10");
    TH1F *h2 = (TH1F*)list->FindObject("fHistMult");
    // h1->Draw();
    // h2->Draw("same");    
...
//***************************************************************************
    ...
    h1->GetXaxis()->SetRangeUser(-2.0, 200); // Only for Eta 
    //h1->Draw("e1");               // Draw h1
    ...
    h2->GetXaxis()->SetRangeUser(0.0, 200);

    // // Define the ratio plot
    TH1D *h3 = (TH1D*)h1->Clone("h3");
    h3->SetLineColor(kRed);   //kBlack
    h3->SetMinimum(0.05);  // Define Y ..
    h3->SetMaximum(0.15); // .. range
    h3->Sumw2();
    ...
    TLine *line = new TLine(gPad->GetUxmin(), 0.1, gPad->GetUxmax(), 0.1);
    line->SetLineStyle(9);
    line->SetLineWidth(2); 
    ...
    TLegend *leg = new TLegend(0.16,0.6,0.45,0.8);  //Make a good position
    //leg->AddEntry(h1,"MC-Reco", "ep");
    //leg->AddEntry(h2,"MC-Truth", "ep");
    leg->AddEntry(h3, "#frac{Sample}{Total Multiplicity}", "ep");
    //of MC-Reco to MC-Truth
    leg->Draw();
    //h3->Draw("ep");       // Draw the ratio plot
    // Save in Pic Folder:======================:
    c->SaveAs("/Users/Helena/Dropbox/0-Speciale/code/panda/Sand_boks/Old/pic/Plots_Multiplicity_sampling/nr10_Ratio_Multiplicity_sampling.png");
    }